...
if(admin_loader == 1)
 ...
 // CATALOG OPTION ENDS
    ...
    $('#confirm-activate .btn-ok').on('click', function(e) {
...
$('button.addProductSubmit-btn').prop('disabled',true);
    $.ajax({
     ...
     success:function(data)
     {
        //console.log(data);
        if ((data.errors)) {
        Dasform.parent().find('.alert-success').hide();
        Dasform.parent().find('.alert-danger').show();
        Dasform.parent().find('.alert-danger ul').html('');
          for(var error in data.errors)
          {
            $('.alert-danger ul').append('<li>'+ data.errors[error] +'</li>')
          }
          Dasform.find('input , select , textarea').eq(1).focus();
        }
        else
        {
          Dasform.parent().find('.alert-danger').hide();
          Dasform.parent().find('.alert-success').show();
          Dasform.parent().find('.alert-success p').html(data);
          Dasform.find('input , select , textarea').eq(1).focus();
        }
          if(admin_loader == 1){
        $('.gocover').hide();
        ...
        //$(this).trigger("reset");
        alert("Product Add was Success, Please Refesh page to add new");
        $('#Dasform')[0].reset();
        $(window).scrollTop(0);
        location.reload();
