<ul>
    ...
    <li class="{{count($category->subs) > 0 ? 'dropdown_list':''}} {{ $loop->index >= 8 ? 'rx-child' : '' }}">
    ...
    @endif
        ...
        <ul class="{{ $category->subs()->withCount('childs')->get()->sum('childs_count') > 0 ? 'my_categories_mega_menu' : 'my_categories_mega_menu column_1' }}">
            @foreach($category->subs()->whereStatus(1)->get() as $subcat)
                <li>
                    ...
                    @if(count($subcat->childs) > 0)
                        <div class="my_categorie_sub_menu">
                            <ul>
