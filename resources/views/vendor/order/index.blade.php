                    ...
                    <div class="table-responsiv">
                        ...
                        <table id="Dastable" class="table table-hover dt-responsive" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                ...
                                @endphp
                                <tr>
                                    <td> <a href="{{ route('vendor-order-invoice', $order->order_number) }}">{{ $order->order->order_number }}</a>
                                    </td>
                                    <td>{{ $qty }}</td>
                                    ...
                                    <td>
                                        ...
                                        <div class="action-list">
                                            ...
                                            <a href="{{ route('vendor-order-show', $order->order->order_number) }}" class="btn btn-primary product-btn"><i class="fa fa-eye"></i>
                                                {{ $langg->lang539 }}</a>
                                            <select id="vendor-status" class="vendor-btn {{ $order->status }}">
                                                <option value="{{ route('vendor-order-status', [$order->order->order_number, 'pending']) }}" {{ $order->status == 'pending' ? 'selected' : '' }}>
                                                    {{ $langg->lang540 }}
                                                </option>

                                                <option value="{{ route('vendor-order-status', [$order->order->order_number, 'processing']) }}" {{ $order->status == 'processing' ? 'selected' : '' }}>
                                                    {{ $langg->lang541 }}
                                                </option>

                                                <option value="{{ route('vendor-order-status', [$order->order->order_number, 'confirmed']) }}" {{ $order->status == 'confirmed' ? 'selected' : '' }}>
                                                    Picked</option>

                                                <option value="{{ route('vendor-order-status', [$order->order->order_number, 'shipped']) }}" {{ $order->status == 'shipped' ? 'selected' : '' }}>Shipped
                                                </option>

                                                <option value="{{ route('vendor-order-status', [$order->order->order_number, 'completed']) }}" {{ $order->status == 'completed' ? 'selected' : '' }}>
                                                    Delivered</option>


                                                <option value="{{ route('vendor-order-status', [$order->order->order_number, 'declined']) }}" {{ $order->status == 'declined' ? 'selected' : '' }}>
                                                    Cancel</option>
                                            </select>
                                        
                                            <a href="javascript:;" data-href="{{$order->order->id}}" class="track" data-toggle="modal" data-target="#modal1"><i class="fas fa-truck"></i> Track Order</a> 
