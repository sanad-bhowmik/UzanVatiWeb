...
@section('content')
                      ...
                      <li>
                        <a href="{{ route('vendor-prod-types') }}">{{ $langg->lang445 }}</a>
	                      ...
	                      <form id="Dasform" action="{{route('vendor-prod-store')}}" method="POST" enctype="multipart/form-data">
	                        {{csrf_field()}}
													...
													<div class="col-lg-7">
													  <!-- <div class="img-upload">
														  <div id="image-preview" class="img-preview" style="background: url({{ asset('assets/vendor/images/upload.png') }});">
															  <label for="image-upload2" class="img-label" id="image-label"><i class="icofont-upload-alt"></i>{{ $langg->lang512 }}</label>
															  <input type="file" name="photo" class="img-upload-p" id="image-upload2" required>
															</div>
															<p class="img-alert mt-2 text-danger d-none"></p>
															<p class="text">Prefered Size: (800x800) or Square Size.</p>
													  </div> -->
													  <input type="file" name="photo" onchange="readURL(this)" accept="image/*" class="custom-file-input" id="gm1">
  ...
  $(document).on('change',"#prod_gallery",function(){
    ...
    $('#Dasform').find('.removegal').val(0);
     ...
     for(var i=0;i<total_file;i++)
     {
      ...
      $('#Dasform').append('<input type="hidden" name="galval[]" id="galval'+i+'" class="removegal" value="'+i+'">')
     }
