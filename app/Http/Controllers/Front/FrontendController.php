...
class FrontendController extends Controller
{
    ...
    function getOS()
    {
        ...
        if (isset($_SERVER['HTTP_USER_AGENT'])) {
            ...
            $user_agent = "Unknown OS Platform";
        }
    ...
    public function index(Request $request)
    {
        ...
        $campaigns = Campaign::where('status', 1)
            ...
            ->orderBy('id', 'desc')
            ->get();
        $shops =    DB::table('users')->where('is_vendor', '=', 2)

            ->leftJoin('vendor_orders', function ($join) {
                $join->on('vendor_orders.user_id', '=', 'users.id');
                $join->Where('vendor_orders.status', '=', 'completed');
            })
            ->selectRaw('users.id,users.shop_name,users.is_vendor,users.shop_number,users.shop_address,users.shop_image, sum(ifnull(vendor_orders.price,0)) as totalSale')
            ->orderBy('totalSale', 'desc')->groupBy('users.id', 'users.shop_name', 'users.shop_number', 'users.shop_address', 'users.shop_image', 'users.is_vendor')
            //    ->toSql();
            ->take(16)
            ->get();
        // dd($shops);

        $sliders = DB::table('sliders')->where('activeStatus', 1)->get();
        $services = DB::table('services')->where('user_id', '=', 0)->get();
        $top_small_banners = DB::table('banners')->where('type', '=', 'TopSmall')->get();
        $feature_products =  Product::where('featured', '=', 1)->where('status', '=', 1)->when(
            $gs->affilate_product == 0,
            function ($q) {
        ...
        return view('front.index', compact('campaigns', 'shops', 'ps', 'sliders', 'services', 'top_small_banners', 'feature_products'));
    }
    ...
    public function extraIndex()
    {
        ...
        $discount_products =  Product::where('is_discount', '=', 1)->where('status', '=', 1)->when(
            $gs->affilate_product == 0,
            function ($q) {
                return $q->where('product_type', '=', 'normal');
            }
        )->orderBy('id', 'desc')->take(8)->get();
        $best_products = Product::where('best', '=', 1)->where('status', '=', 1)->when(
            $gs->affilate_product == 0,
            function ($q) {
                return $q->where('product_type', '=', 'normal');
            }
        )->orderBy('id', 'desc')->take(8)->get();
        $top_products = Product::where('top', '=', 1)->where('status', '=', 1)->when(
            $gs->affilate_product == 0,
            function ($q) {
                return $q->where('product_type', '=', 'normal');
            }
        )->orderBy('id', 'desc')->take(8)->get();;
        $big_products = Product::where('big', '=', 1)->where('status', '=', 1)->when(
            $gs->affilate_product == 0,
            function ($q) {
                return $q->where('product_type', '=', 'normal');
            }
        )->orderBy('id', 'desc')->take(8)->get();;
        $hot_products =  Product::where('hot', '=', 1)->where('status', '=', 1)->when(
            $gs->affilate_product == 0,
            function ($q) {
                return $q->where('product_type', '=', 'normal');
            }
        )->orderBy('id', 'desc')->take(9)->get();
        $latest_products =  Product::where('latest', '=', 1)->where('status', '=', 1)->when(
            $gs->affilate_product == 0,
            function ($q) {
                return $q->where('product_type', '=', 'normal');
            }
        )->orderBy('id', 'desc')->take(9)->get();
        $trending_products =  Product::where('trending', '=', 1)->where('status', '=', 1)->when(
            $gs->affilate_product == 0,
            function ($q) {
                return $q->where('product_type', '=', 'normal');
            }
        )->orderBy('id', 'desc')->take(9)->get();
        $sale_products =  Product::where('sale', '=', 1)->where('status', '=', 1)->when(
            $gs->affilate_product == 0,
            function ($q) {
                return $q->where('product_type', '=', 'normal');
            }
    ...
    public function autosearch($slug)
    {
        if (mb_strlen($slug, 'utf-8') > 1) {
            $search = ' ' . $slug;
            $prods = Product::where([['name', 'like', '%' . $search . '%'], ['status', '=', 1]])
                ->orWhere([['name', 'like', $slug . '%'], ['status', '=', 1]])
                ->take(10)->get()->reject(function ($item) {
    ...
    public function trackload($id)
    {
        ...
        $datas = array('Pending', 'Processing', 'Picked', 'Shipped', 'Delivered');
    ...
    public function upazilas($id)
    {
        ...
        $upazila = DB::table('upazilas')->where('district_id', $id)->get();
    ...
    public function allShops(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return view('includes.filtered-shops', $data);
        }


        return view('front.all-shops', $data);
    }
}
