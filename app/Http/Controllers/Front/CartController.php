   ...
   public function addtocart($id)
    {
       ...
       $prod = Product::where('id','=',$id)->first();
        ...
        if($prod->campaign_product ==1){

            if($prod->campaign->status != 1){
                if( Session::has('cart')){
                    Session::forget('cart');
                }
                return redirect('/'); 
       ...
       // new by das to restrict user from adding product from multiple shop 24/8/2021
      // return $prod->user_id;
        //return $cart->items;
        if(!empty($cart->items)){

            foreach ($cart->items as $prods) {
                $user[] = $prods['item']['user_id'];
            }
           
            if(!in_array($prod->user_id,$user)){
               // return 2;
   ...
   public function addcart($id)
    {
       ...
       // return response()->json($prod->campaign->status);   
        // Set Attrubutes
    ...
    //new by das 24-8-2021 add to buy now
    public function addtobuynowcart()
    {
        if( Session::has('buynowcart')){
       ...
       // new by das to restrict user from adding product from multiple shop 24/8/2021
        ...
        return redirect()->route('front.buynow');      
    }  
    ...
    public function couponcheckreload()
    {
