...
class CatalogController extends Controller
{
    ...
    public function category(Request $request, $slug=null, $slug1=null, $slug2=null)
    {
      ...
      $prods = Product::when($cat, function ($query, $cat) {
                                  ...
                                  $prods = $prods->where('status', 1)->where('campaign_product',0)->orderBy('id','desc')->get();
      $prods = (new Collection(Product::filterProducts($prods)))->paginate(32);
    ...
    {
        $this->code_image();
        $productt = Product::where('slug','=',$slug)->where('campaign_product',0)->where('status',1)->firstOrFail();
        $productt->views+=1;
        $productt->update();
        if (Session::has('currency'))
        {
...
//        $product->update();
    ...
    public function productSearch(Request $request){
      ...
      $input = strip_tags($request->q);
      $shop = strip_tags($request->shop_id);
      $data = Product::where([['name', 'LIKE', '%' .$input. '%'],['user_id','=',$shop],['status','=',1],['campaign_product','=',0]])
      ->orWhere([['slug', 'LIKE', '%' .$input. '%'],['user_id','=',$shop],['status','=',1],['campaign_product','=',0]])
      ->orWhere([['sku', 'LIKE', '%' .$input. '%'],['user_id','=',$shop],['status','=',1],['campaign_product','=',0]])
      ->orWhere([['meta_tag', 'LIKE', '%' .$input. '%'],['user_id','=',$shop],['status','=',1],['campaign_product','=',0]])
      ->orWhere([['meta_description', 'LIKE', '%' .$input. '%'],['user_id','=',$shop],['status','=',1],['campaign_product','=',0]])
 //     ->toSql();
      ->get();
