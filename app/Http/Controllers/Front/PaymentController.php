...
class PaymentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($curr->name != 'USD'){
             return redirect()->back()->with('unsuccess',"Invalid Currency For Paypal Payment. ");  
        }
        ...
        $order['item_amount'] = round($request->total / $curr->value, 2);
        ...
        if ($result->getState() == 'approved') {
            ...
            foreach($cart->items as $key => $prod)
            {
                if(!empty($prod['item']['license']) && !empty($prod['item']['license_qty']))
                {
                    ...
                    $order['wallet_price'] = round($paypal_data['wallet_price']/ $curr->value, 2);  
                    ...
                    {
                        $val = $order_data['item_amount'] / $curr->value;
                        $val = $val / 100;
