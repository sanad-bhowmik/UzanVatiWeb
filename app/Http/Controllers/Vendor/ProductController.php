...
class ProductController extends Controller
{
    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ->editColumn('name', function(Product $data) {
                                $imgThumb = $data->thumbnail;
                                $path =asset('assets/images/thumbnails/'.$imgThumb);
                                $img = '<img width=100 height=100 alt="no-image" src="'.$path.'" />';
                                ...
                                return  $img.'<br>'.$name.'<br>'.$id;
                            })
                            ->editColumn('category', function(Product $data) {
                                $cname = $data->category->name ;
                                return  $cname;
                            })
                            ->editColumn('sub-category', function(Product $data) {
                                $subcname = $data->subcategory->name ;
                                return  $subcname;
                            })
    ...
    public function datatablescampaign()
    {
    	 $user = Auth::user();
         ...
         return Datatables::of($datas)
                            ...
                            ->editColumn('campaign_id', function(Product $data) {
                            ...
                            ->addColumn('action', function(Product $data) {
                                return '<div class="action-list"><a href="' . route('vendor-prod-edit-campaign',$data->id) . '"> <i class="fas fa-edit"></i>'.$this->vendor_language->lang715.'</a><a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="'.$data->id.'"><i class="fas fa-eye"></i> '.$this->vendor_language->lang716.'</a><a href="javascript:;" data-href="' . route('vendor-prod-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
                            })
                            ->rawColumns(['name', 'status', 'action'])
                            ->toJson(); //--- Returning Json Data To Client Side
    }

    public function datatablesshop()
    {
    	 $user = Auth::user();
         $datas = $user->products()->where('product_type','normal')
    ...
    */
    ...
    {
        return view('vendor.product.index-campaign');
    }


    public function indexshop()
    {
        return view('vendor.product.index-shop');
    }
...
//                $input['childcategory_id'] = $request->childcategory_id;
        ...
        //--- Logic Section
            ...
            {      
              ...
              // $file->move('assets/images/products',$name);           
               ...
               $image_name = time().Str::random(8).'.png';
               $img->save(public_path().'/assets/images/products/'.$image_name);
               $input['photo'] = $image_name;
    ...
    public function edit($id)
    {
        ...
        // else
            return view('vendor.product.edit.physical',compact('cats','data','sign','brands'));
    }
    ...
    public function editcampaign($id)
    {
        ...
        $sign = Currency::where('is_default','=',1)->first();
        $brands = Brand::where('status',1)->get();
        $campaigns =DB::table('campaign_join_requests')
        ...
        ->selectRaw('campaigns.id,campaigns.title,campaigns.banner,campaigns.name,campaigns.start_date,campaigns.end_date,campaigns.file')
        ->orderBy('campaign_join_requests.id', 'desc')->get();
        ...
        //     return view('vendor.product.edit.license',compact('cats','data','sign'));
        // else
            return view('vendor.product.edit.physical-campaign',compact('cats','data','sign','brands','campaigns'));
    }
...
public function addproductfromlist(Request $request){
    ...
    if($request->exists('letter')){
        $datas = InventoryProduct::where('status',1)->where('name','like',$request->letter.'%')->where('campaign_product',0)->orderBy('id','desc')->get();
    }else{
        $datas = InventoryProduct::where('status',1)->where('campaign_product',0)->limit(100)->orderBy('id','desc')->get();
   
    }

    return view('vendor.product.create.from-list',compact('datas',$datas));

}

public function addproductfromlistcampaign(Request $request){
    
    if($request->exists('letter')){
        $datas = InventoryProduct::where('status',1)->where('name','like',$request->letter.'%')->orderBy('id','desc')->get();
    }else{
        $datas = InventoryProduct::where('status',1)->limit(100)->orderBy('id','desc')->get();
   
    }

    return view('vendor.product.create.from-list-campaign',compact('datas',$datas));
...
public function productlist(Request $request){
    ...
    $datas = DB::table('inventory_products')
    ->leftJoin('brands','inventory_products.brand_id','=','brands.id')
    ->where('brands.brand_name','=',strtoupper($request->products))
    ->orWhere('name','like','%'. $request->products .'%')
    ->where('campaign_product',0)
    ...
    return view('vendor.product.create.product-list',compact('datas',$datas));

}


public function productlistcampaign(Request $request){
    ...
    ->get();
 // dd($datas); 
    return view('vendor.product.create.product-list-campaign',compact('datas',$datas));
...
{   
    $userId= Auth::user()->id;
    $inventoryProduct = InventoryProduct::where('id',$request->id)->where('status',1)->firstOrFail();
    $product = Product::where('inventory_product_id',$request->id)->where('campaign_product',0)->where('user_id',Auth::user()->id)->get();
    ...
    $newProduct->sku=$inventoryProduct->sku;
    $newProduct->product_type=$inventoryProduct->product_type;
    $newProduct->user_id = $userId;
    $newProduct->category_id = $inventoryProduct->category_id;
    $newProduct->subcategory_id =$inventoryProduct->subcategory_id;
    $newProduct->childcategory_id =$inventoryProduct->childcategory_id;
    $newProduct->inventory_product_id =$inventoryProduct->id;
    //$newProduct->attributes =$inventoryProduct->attributes;
    $newProduct->name =$inventoryProduct->name ;
    $newProduct->slug  =$inventoryProduct->slug.time() ;
    $newProduct->photo =$inventoryProduct->photo;
    $newProduct->thumbnail =$inventoryProduct->thumbnail;
   // $newProduct->file =$inventoryProduct->file ;
    $newProduct->size =$inventoryProduct->size ;
    $newProduct->size_qty =$inventoryProduct->size_qty ;
    $newProduct->size_price =$inventoryProduct->size_price;
    $newProduct->color =$inventoryProduct->color;
    $newProduct->price =$inventoryProduct->price;
    $newProduct->previous_price =$inventoryProduct->previous_price;
    $newProduct->details =$inventoryProduct->details;
    $newProduct->brand_id =$inventoryProduct->brand_id ;
    $newProduct->stock =0;
    $newProduct->status=1;
    $newProduct->campaign_product=0;
   // $newProduct->features =$inventoryProduct->features;
   // $newProduct->colors =$inventoryProduct->colors;
    $newProduct->type =$inventoryProduct->type;
    $newProduct->best =$inventoryProduct->best;
    $newProduct->top =$inventoryProduct->top;
    $newProduct->hot  =$inventoryProduct->hot;
    $newProduct->big   =$inventoryProduct->big;
    $newProduct->trending    =$inventoryProduct->trending ;
    $newProduct->sale    =$inventoryProduct->sale ;
    $newProduct->is_catalog   =$inventoryProduct->is_catalog ;
    $newProduct->catalog_id   =$inventoryProduct->catalog_id;
    //dd(DB::getQueryLog());
...
public function storefromlistcampaign(Request $request)
{   
    $userId= Auth::user()->id;
    $inventoryProduct = InventoryProduct::where('id',$request->id)->where('status',1)->firstOrFail();
    $product = Product::where('inventory_product_id',$request->id)->where('campaign_product',1)->where('user_id',Auth::user()->id)->get();
