...
class VendorController extends Controller
{
    ...
    {
        $user = Auth::user();  
        $pending = VendorOrder::where('user_id','=',$user->id)->where('status','=','pending')
        ->groupBy('order_id')
        ->get(); 
        $processing = VendorOrder::where('user_id','=',$user->id)->where('status','=','processing')
        ->groupBy('order_id')
        ->get(); 
        $completed = VendorOrder::where('user_id','=',$user->id)->where('status','=','completed')
        ->groupBy('order_id')
        ->get(); 


        $confirmed = VendorOrder::where('user_id','=',$user->id)->where('status','=','confirmed')
        ->groupBy('order_id')
        ->get(); 

        $declined = VendorOrder::where('user_id','=',$user->id)->where('status','=','declined')
        ->groupBy('order_id')
        ->get(); 


        $shipped = VendorOrder::where('user_id','=',$user->id)->where('status','=','shipped')
        ->groupBy('order_id')
        ->get(); 


        return view('vendor.index',compact('user','pending','processing','completed','shipped','confirmed','declined'));
    }
    ...
    public function mercadoLoad(){
        return view('load.vpayment');
    }
    ...
    public function vendorrequest($id)
    {
        ...
        return view('vendor.plans.details',compact('user','subs','package','curr'));
    }
