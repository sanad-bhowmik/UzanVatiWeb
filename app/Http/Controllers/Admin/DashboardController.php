...
class DashboardController extends Controller
{
    ...
    {
        $pending = Order::where('status','=','pending')->where('user_id','=',0)->get();
        $confirmed = Order::where('status','=','confirmed')->where('user_id','=',0)->get();
        $shipped = Order::where('status','=','shipped')->where('user_id','=',0)->get();
        $processing = Order::where('status','=','processing')->where('user_id','=',0)->get();
        $completed = Order::where('status','=','completed')->where('user_id','=',0)->get();
        $pendingShop = Order::where('status','=','pending')->where('user_id','!=',0)->get();
        $confirmedShop = Order::where('status','=','confirmed')->where('user_id','!=',0)->get();
        $shippedShop = Order::where('status','=','shipped')->where('user_id','!=',0)->get();
        $processingShop = Order::where('status','=','processing')->where('user_id','!=',0)->get();
        $completedShop = Order::where('status','=','completed')->where('user_id','!=',0)->get();
        $canceledShop = Order::where('status','=','declined')->where('user_id','!=',0)->get();
        ...
        $users = User::all();
        $products = Product::where('user_id','=',0)->get();;
        $productsShop = Product::where('user_id','!=',0)->get();;
        $blogs = Blog::all();
        ...
        return view('admin.dashboard',compact('shipped','shippedShop','confirmed','confirmedShop','canceledShop','productsShop','pendingShop','processingShop','completedShop','pending','activation_notify','processing','completed','products','users','blogs','days','sales','pproducts','rorders','poproducts','rusers','referrals','browsers'));
    }
    ...
    public function activation_submit(Request $request)
    {
        ...
        $varUrl = str_replace (' ', '%20', config('services.Das.ocean').'purchase112662activate.php?code='.$purchase_code.'&domain='.$my_domain.'&script='.$my_script);
    ...
    public function movescript(){
        ...
        $files = public_path();
        $bkupname = 'Das-By-DasOcean-'.date('Y-m-d').'.zip';
