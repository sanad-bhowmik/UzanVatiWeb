...
class ProductController extends Controller
{
    ...
    {
        $datas = InventoryProduct::where('product_type', '=', 'normal')->where('status', 1)->orderBy('id', 'desc')->get();

        //--- Integrating This Collection Into Datatables
        return Datatables::of($datas)
            ->editColumn('name', function (InventoryProduct $data) {
            ...
            ->editColumn('category', function (InventoryProduct $data) {
                $cname = $data->category->name;
                return  $cname;
            })
            ->editColumn('sub-category', function (InventoryProduct $data) {
                $subcname = $data->subcategory->name;
                return  $subcname;
            })
            ->editColumn('price', function (InventoryProduct $data) {
                $sign = Currency::where('is_default', '=', 1)->first();
                $price = round($data->price * $sign->value, 2);
                $price = $sign->sign . $price;
                return  $price;
            })
            ->editColumn('stock', function (InventoryProduct $data) {
            ...
            ->addColumn('status', function (InventoryProduct $data) {
            ...
            ->addColumn('action', function (InventoryProduct $data) {
    ...
    {
        $datas = InventoryProduct::where('product_type', '=', 'normal')
    ...
    public function datatablesShop()
    {
        ...
        ->editColumn('name', function(Product $data) {
            ...
            ->editColumn('vendor', function (Product $data) {
                ...
                $vendor = $data->user_id != 0 ? '<br><small class="ml-2"> <a href="' . route('admin-vendor-show', $data->user_id) . '" target="_blank">' . $data->user->shop_name . '</a></small>'  : '';
            ...
            ->rawColumns(['name', 'vendor', 'status', 'action'])
            ->toJson(); //--- Returning Json Data To Client Side
    }
    ...
    {
        $datas = InventoryProduct::where('status', '=', 0)->orderBy('id', 'desc')->get();
        //--- Integrating This Collection Into Datatables
        return Datatables::of($datas)
        ->editColumn('name', function(InventoryProduct $data) {
            $imgThumb = $data->thumbnail;
    ...
    //*** JSON Request
    public function deactivedatatablesShop()
    {
    ...
    public function catalogdatatablesShop()
    {
        $datas = Product::where('is_catalog', '=', 1)->orderBy('id', 'desc')->get();
    ...
    {
        return view('admin.product.indexCampaign');
    }
    ...
    {
        return view('admin.product.indexShop');
    }
    ...
    {
        $data = InventoryProduct::findOrFail($id1);
        $data->status = $id2;
        $data->update();
    }
    ...
    public function store(Request $request)
    {
      ...
      //  dd($input['campaign_product']);
        ...
        */
        ...
        $img  = Image::make($image->getRealPath())->resize(800, 800);
        $image_name = time() . Str::random(8) . '.png';
        $img->save(public_path() . '/assets/images/products/' . $image_name);
        //dd(public_path().'/assets/images/galleries/'.$image_name);
        $input['photo'] = $image_name;
        // dd($img);
        ...
        $input['price'] = round(($input['price'] / $sign->value));
        $input['previous_price'] = round(($input['previous_price'] / $sign->value));
        ...
        // Set SLug
        $prod = InventoryProduct::find($data->id);
        if ($prod->type != 'Physical') {
            $pname = $this->clean($data->name);
            $prod->slug = Str::slug($pname, '-') . '-' . strtolower(Str::random(3) . $data->id . Str::random(3));
            $prod->sku = strtolower(Str::random(3) . $data->id . Str::random(3));
        } else {
            $pname = $this->clean($data->name);
            $prod->slug = Str::slug($pname, '-') . '-' . strtolower(Str::random(3) . $data->id . Str::random(3));
            $prod->sku  = strtolower(Str::random(4) . $data->id . Str::random(4));
        }

        // Set Thumbnail
        //  dd(public_path().'/assets/images/products/'.$prod->photo);
    ...
    public function edit($id)
    {
        ...
        elseif ($data->type == 'License')
            ...
            return view('admin.product.edit.physical', compact('cats', 'data', 'sign', 'brands'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        Product::where('inventory_product_id', $id)
            ->update([
